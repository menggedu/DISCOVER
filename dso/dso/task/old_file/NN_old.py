...
def model_NN(x_train, y_train,
    ...
    for epoch in range(max_epoch):
        ...
        if (epoch+1)%display_step == 0:
            torch.save(model.state_dict(), ckpt_path+f'{epoch+1}_remote.ckpt') 
            model.eval()
...
if __name__ == '__main__':
    ...
    ckpt_path = f'./dso/task/pde/ckpt/{dataset}/'
    ...
    if mode == 0:
        ...
        for i in range(1,100):
            epoch = i*1000 
        # i='latest'
            ckpt_path2= f'{ckpt_path}{epoch}.ckpt'
            ...
            if best_rmse>cur_rmse:
                best_rmse = cur_rmse
                best_epoch = epoch
            print(f'Epoch: {epoch}, The rmse of raw data  is {cur_rmse}:')
                
            y_all_pred = y_all_pred.reshape(n,m)
        print(f"best epcoh is {best_epoch}, best rmse is {best_rmse}")

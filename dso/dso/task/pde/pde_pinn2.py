...
class PDEPINNTask(PDETask):
    ...
    def __init__(self, function_set, dataset, metric="residual",
        ...
        self.ut = [self.ut[i].reshape(-1,1) for i in range(len(self.ut))]
        self.ut_true =  [self.ut_true[i].reshape(-1,1) for i in range(len(self.ut_true))]
    ...
    def AD_generate_2D(self,model):
        ...
        self.ut = [tensor2np(ut[i]) for i in range(len(ut))]

...
def model_NN(x_train, y_train,
    ...
    display_step = 100 #int(max_epoch/100)
    ...
    for epoch in range(max_epoch):
        ...
        if (epoch+1)%display_step == 0:
            torch.save(model.state_dict(), ckpt_path+f'{epoch+1}_32.ckpt') 
            model.eval()
            y_test_pred = model(x_test)
            test_loss = criterion(y_test_pred, y_test)
            logging.info('step %d, loss= %.6f, test_loss= %.6f'%(epoch+1, loss.data.cpu(),test_loss.data.cpu() ))
            model.train()
...
def eval_result (y_test, y_pred, y_train, y_pred_train):
    ...
    logging.info('The std(y_pred_train) is:',np.std(y_pred_train))
    if len(y_test) == 0:
        RMSE = 0
    else:
        RMSE = mean_squared_error(y_test, y_pred) ** 0.5
    RMSE_train = mean_squared_error(y_train, y_pred_train) ** 0.5
    logging.info('The RMSE of prediction is:', RMSE)
    logging.info('The RMSE of prediction of training dataset is:', RMSE_train)
    if len(y_test) == 0:
        R_square = 0
    else:
...
if __name__ == '__main__':
    ...
    if mode == 0:
        ...
        for i in range(1,100):
            i = i*500
        # i='latest
            ckpt_path2= f'{ckpt_path}{i}_32.ckpt'
            data_path = f'{data_dir}/{dataset}_data_amount{ratio}_noise{data_noise_level}.npy'
